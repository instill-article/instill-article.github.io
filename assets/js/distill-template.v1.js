// (VERY HACKY) modified version of the Distill template
// (https://distill.pub/template.v1.js). Modifications:
// - Remove automatically generated OpenGraph and Twitter tags
(function (global, factory) {
  ...
  function encodeHex(character) {
    ...
    return (
      "\\" + handle + common$7.repeat("0", length - string.length) + string
    );
  }
      ...
      if (parsed) {
        parsed.forEach(function (e) {
          for (var k in e.entryTags) {
            ...
            val = val.replace(/[\t\n ]+/g, " ");
            val = val.replace(/{\\["^`\.'acu~Hvs]( )?([a-zA-Z])}/g, function (
              full,
              x,
              char
            ) {
  ...
  var expandData = function (dom, data) {
    ...
    if (data.authors.length > 2) {
      ...
      data.concatenatedAuthors =
        data.authors[0].lastName + " & " + data.authors[1].lastName;
    } else if (data.authors.length === 1) {
      data.concatenatedAuthors = data.authors[0].lastName;
    }

    data.bibtexAuthors = data.authors
      .map(function (author) {
        return author.lastName + ", " + author.firstName;
      })
      .join(" and ");

    data.slug = data.authors.length
      ? data.authors[0].lastName.toLowerCase() +
        data.publishedYear +
  ...
  var meta = function (dom, data) {
    ...
    function meta(name, content, force) {
      if (content || force) {
        appendHead(
          '    <meta name="' + name + '" content="' + index$1(content) + '" >\n'
        );
      }
    }
      ...
      if (data.citations) {
        data.citations.forEach(function (key) {
          ...
          if (!d) {
            ...
            meta(
              "citation_reference",
              citation_meta_content(data.bibliography[key])
            );
          }
        });
      }
  ...
  function citation_meta_content(ref) {
    ...
    var arxiv_id_search = /https?:\/\/arxiv\.org\/pdf\/([0-9]*\.[0-9]*)\.pdf/.exec(
      ref.url
    );
    arxiv_id_search =
      arxiv_id_search ||
  ...
  var appendix = function (dom, data) {
    ...
    if (el) {
      ...
      if (dom.querySelector("dt-cite")) {
        newHTML =
          newHTML + "<h3>References</h3><dt-bibliography></dt-bibliography>";
      }
  ...
  var citation = function (dom, data) {
    ...
    var appendCiteHoverDiv = (function () {
      ...
      var hover_boxes_container = nodeFromString(
  ...
  symbolNode.prototype.tryCombine = function (sibling) {
    ...
    for (var style in this.style) {
      if (
        this$1.style.hasOwnProperty(style) &&
 ...
 */
  ...
  var mathrmSize = function (value, size, mode, options) {
    return buildCommon$4.makeSymbol(
      value,
      "Size" + size + "-Regular",
      mode,
      options
  ...
  var makeLargeDelim = function (delim, size, center, options, mode, classes) {
    ...
    if (center) {
      var shift =
        (1 - options.style.sizeMultiplier) * options.style.metrics.axisHeight;
  ...
  var makeSizedDelim = function (delim, size, options, mode, classes) {
    ...
    if (
      utils$6.contains(stackLargeDelimiters, delim) ||
   ...
   */
  var makeCustomSizedDelim = function (
    delim,
    height,
    center,
    options,
   ...
   */
  var makeLeftRightDelim = function (
    delim,
    height,
    depth,
    options,
  ...
  groupTypes.text = function (group, options) {
    ...
    return makeSpan$1(
      ["mord", "text", newOptions.style.cls()],
      inner,
      newOptions
    );
  };
  ...
  groupTypes.genfrac = function (group, options) {
    ...
    var numerreset = makeSpan$1(
      [style.reset(), nstyle.cls()],
      [numer],
      newOptions
    );
    ...
    var denomreset = makeSpan$1(
      [style.reset(), dstyle.cls()],
      [denom],
      newOptions
    );
    ...
    if (group.value.hasBarLine) {
      ruleWidth =
        fontMetrics.metrics.defaultRuleThickness / options.style.sizeMultiplier;
    } else {
      ruleWidth = 0;
    }
    ...
    if (ruleWidth === 0) {
      ...
      if (numShift - numer.depth - (axisHeight + 0.5 * ruleWidth) < clearance) {
        numShift +=
          clearance - (numShift - numer.depth - (axisHeight + 0.5 * ruleWidth));
      }

      if (
        axisHeight - 0.5 * ruleWidth - (denom.height - denomShift) <
        clearance
      ) {
        denomShift +=
          clearance -
  ...
  groupTypes.spacing = function (group, options) {
    if (
      ...
      // generate these.
      return makeSpan$1(
        ["mspace", buildCommon$1.spacingFunctions[group.value].className],
        [],
        options
      );
    }
  ...
  groupTypes.op = function (group, options) {
    ...
    if (hasLimits) {
      ...
      if (!supGroup) {
        ...
        bottom =
          fontMetrics.metrics.bigOpSpacing5 +
          submid.height +
          submid.depth +
          subKern +
          base.depth +
  ...
  groupTypes.mod = function (group, options) {
    ...
    if (group.value.modType !== "pod") {
      ...
      if (group.value.modType === "bmod") {
        ...
        if (!options.style.isTight()) {
          inner.push(
            makeSpan$1(["mspace", "negativemediumspace"], [], options)
          );
        }
      } else {
  ...
  groupTypes.katex = function (group, options) {
    ...
    var k = makeSpan$1(
      ["k"],
      [buildCommon$1.mathsym("K", group.mode)],
      options
    );
    var a = makeSpan$1(
      ["a"],
      [buildCommon$1.mathsym("A", group.mode)],
      options
    );
    ...
    var t = makeSpan$1(
      ["t"],
      [buildCommon$1.mathsym("T", group.mode)],
      options
    );
    var e = makeSpan$1(
      ["e"],
      [buildCommon$1.mathsym("E", group.mode)],
      options
    );
    ...
    var x = makeSpan$1(
      ["x"],
      [buildCommon$1.mathsym("X", group.mode)],
      options
    );
  ...
  groupTypes.overline = function (group, options) {
    ...
    var ruleWidth =
      fontMetrics.metrics.defaultRuleThickness / style.sizeMultiplier;
      ...
      for (i = 1; i < inner.length; i++) {
        if (inner[i].isMiddle) {
          ...
          inner[i] = delimiter.leftRightDelim(
            inner[i].isMiddle.value,
            innerHeight,
            innerDepth,
            inner[i].isMiddle.options,
            group.mode,
            []
          );
        }
  ...
  groupTypes.accent = function (group, options) {
    ...
    var accentBody = makeSpan$1(
      ["accent-body", vecClass],
      [makeSpan$1([], [accent])]
    );
  ...
  var buildGroup = function (group, options) {
    ...
    if (groupTypes[group.type]) {
      ...
      if (options.style !== options.parentStyle) {
        multiplier =
          options.style.sizeMultiplier / options.parentStyle.sizeMultiplier;
      ...
      if (options.size !== options.parentSize) {
        multiplier =
          buildCommon$1.sizingMultiplier[options.size] /
  ...
  groupTypes$1.spacing = function (group) {
    ...
    if (
      ...
      node.setAttribute(
        "width",
        buildCommon$5.spacingFunctions[group.value].size
      );
    }
  ...
  groupTypes$1.op = function (group, options) {
    ...
    if (group.value.symbol) {
      ...
      node = new mathMLTree.MathNode(
        "mo",
        buildExpression$1(group.value.value, options)
      );
    } else {
  ...
  groupTypes$1.sizing = function (group, options) {
    ...
    node.setAttribute(
      "mathsize",
      buildCommon$5.sizingMultiplier[group.value.size] + "em"
    );
  ...
  Parser$1.prototype.parseGroup = function (optional) {
    ...
    if (this.nextToken.text === (optional ? "[" : "{")) {
      ...
      return new ParseFuncOrArgument(
        new ParseNode("ordgroup", expression, this.mode, firstToken, lastToken),
        false
      );
    } else {
  ...
  Parser$1.prototype.parseSymbol = function () {
    ...
    if (functions[nucleus.text]) {
      ...
      return new ParseFuncOrArgument(
        new ParseNode(
          symbols$5[this.mode][nucleus.text].group,
          nucleus.text,
          this.mode,
          nucleus
        ),
  ...
  var DTMath = function (dom, data) {
    ...
    equationElements.forEach(function (el) {
      ...
      el.innerHTML =
        html$3 + katex.renderToString(content, { displayMode: displayMode });
    });
  };
...
********************************************** */
  ...
  var generateCrossref = function (data) {
    ...
    var batch_id = data.authors.length
      ? data.authors[0].lastName.toLowerCase().slice(0, 20)
      : "Anonymous";
    batch_id += "_" + date.getFullYear();
    batch_id +=
      "_" +
      data.title.split(" ")[0].toLowerCase().slice(0, 20) +
      "_" +
      batch_timestamp;
    // generate XML
    var crf_data = {
              ...
              journal: [
                {
                  journal_metadata: [
                    ...
                    {
                      abbrev_title:
                        data.journal.abbrev_title ||
                        data.journal.title ||
                        data.journal.full_title,
                    },
